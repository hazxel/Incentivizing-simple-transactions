...
def plot_operation(original_throughput, simplified_throughput):
    rounds = len(original_throughput)
    samples = len(original_throughput[0])
    ...
    for samples in original_throughput:
        ...
        std = np.std(samples)
        original_mean.append(mean)
        original_ceil.append(mean + std)
        original_floor.append(mean - std)
    for samples in simplified_throughput:
        mean = np.mean(samples)
        std = np.std(samples)
        simplified_mean.append(mean)
        simplified_ceil.append(mean + std)
        simplified_floor.append(mean - std)
